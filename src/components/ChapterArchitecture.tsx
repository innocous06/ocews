...
export const ChapterArchitecture: React.FC = () => {
  ...
  return (
    <section id="architecture" className="py-12 border-t border-neutral-900 max-w-6xl mx-auto px-4 sm:px-6">
      {/* Chapter Marker */}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 my-8 font-mono text-xs">
        {steps.map(step => (
          <div
            key={step.num}
            className="border border-neutral-800 bg-obsidian-950 p-4 rounded-xl flex flex-col justify-between"
          >
            <div>
              <div className="flex items-center justify-between border-b border-neutral-900 pb-2 mb-3">
                <span className="text-amber-400 font-bold">STAGE {step.num}</span>
                <span className="text-[10px] text-neutral-500 uppercase">{step.volume}</span>
              </div>
              <div className="text-sm font-bold text-neutral-100 mb-0.5">{step.name}</div>
              <div className="text-[11px] text-neutral-400 italic mb-2 font-serif">{step.sub}</div>
